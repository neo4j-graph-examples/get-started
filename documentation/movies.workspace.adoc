...
[source,cypher]
----
MATCH (movie:Movie) WHERE movie.released >= 1990 AND movie.released < 2000
...
[source,cypher]
----
MATCH (movie:Movie {title: "Cloud Atlas"})<-[:DIRECTED]-(director:Person)
...
[source,cypher]
----
MATCH (person:Person)-[relationship]-(:Movie {title: "Cloud Atlas"})
RETURN person.name, type(relationship), relationship
----
...
[source,cypher]
----
MATCH (:Person {name:"Kevin Bacon"})-[*1..6]-(hollywood)
...
[source,cypher]
----
MATCH (actor:Person {name:"Tom Hanks"})-[:ACTED_IN]->(m)<-[:ACTED_IN]-(coActors),
...
----
MATCH (actor:Person {name:"Tom Hanks"})-[:ACTED_IN]->(m)<-[:ACTED_IN]-(coActors),
  (coActors)-[:ACTED_IN]->(m2)<-[:ACTED_IN]-(actor2:Person {name:"Keanu Reeves"})
RETURN m, coActors, m2
----
