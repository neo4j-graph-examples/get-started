...
Use the command below but be careful to only run it **once** as it will add data every time it is run.
...
To start, can you find the actor Tom Hanks?
...
The following query lets you see all movies and actors up to six hops away from Kevin Bacon:
...
Cypher has a built-in algorithm for this, `ShortestPath` and it looks like this:
...
The `WHERE NOT` part of the query ensures that you don't get actors who already worked with Tom Hanks nor Tom Hanks himself.
The results are ordered by the number of co-co-actors they have in common.

Now, if you think it would be a good idea for Tom Hanks and Keanu Reeves to do a movie together, who would be able to introduce them?

[source,cypher]
----
MATCH (tom:Person {name:"Tom Hanks"})-[:ACTED_IN]->(m)<-[:ACTED_IN]-(coActors),
  (coActors)-[:ACTED_IN]->(m2)<-[:ACTED_IN]-(keanu:Person {name:"Keanu Reeves"})
RETURN tom, m, coActors, m2, keanu
----
...
Note that this does in fact permanently delete the nodes and relationships in this database, so use with caution.
