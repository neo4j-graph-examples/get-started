...
You will use the Query tool to explore connections between actors, directors, and movies, and also solve the famous Bacon-path.
...
To start, can you find the actor _Tom Hanks_ which is a `Person` in the graph?
...
You can return _all_ relationship properties if you are not sure exactly what you are looking for:
