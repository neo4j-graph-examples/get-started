...
You will learn your way around the Query tool's UI so you can make the most of all it has to offer.
...
If not, then first navigate to _Import_:

button::Go to Import[role=NX_TAB_NAV,tab=import]
...
With the data imported, navigate back to the _Query_ tool and get started with learning Cypher:
