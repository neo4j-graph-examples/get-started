...
This guide focuses on loading data from CSV, if loading data from a relationsal database, refer to the guide _Import from Relational Database_.
...
To get started, navigate to the _Import_ tab under _Data Services_ section and choose the CSV option when adding a data source. Then {northwind-subset-data-only-no-model-zip-file}[download and unzip this file of CSVs^] and drop the files onto the highlight:import/import-file-panel[data source panel].
