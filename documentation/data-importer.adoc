...
Data Importer is designed to get you started with Neo4j as quickly as possible and is recommend for new users and those with rapid prototyping needs. 
...
Configuring Data Importer for import is a simple three-stage-process: (1) providing your flat files, (2) defining a Graph Model and (3) mapping that data to those flat files: 
...
While some flat files might refer to nodes and relationshps, more often than not, the nodes and relationships will be implicitly represented. 
The mapping stage is all about helping Data Importer understand which parts of which files represent the nodes and relationships in the graph model. This gives Data Importer all the infomration it needs to run an Import job.  
...
For convenience, click the button to have a subset of the Northwind dataset loaded for you, so you don't have to drag and drop them.
(You will work with the full set later on, but for now this is sufficient.)
...
You should now see three CSV files in the _Files_ section.
...
You are likely familiar with the _graph data model_ by now, so you know it determines how your data is connected.
...
You could include `quantity`, `unitPrice` and `discount` as properties relevant to the relationship. 
For example `quantity` does not belong on either the `Product` or `Order` nodes since products the will be sold in different quantities on different orders.  
...
- Q: Missing files - Why does data importer say I need to provide my files after I've aready provided them? 
Why does it happen so often? 
A: When you provide Data Importer with your files, you are actually providing your Web Browser with a link to those files on your local filesystem, they aren't uploaded anywhere - Data importer stream the content of the files to you database only when you run the import. 
...
A: Data Importer doesn't currently allow you to swap out files in the UI. 
...
- Q: How do I change the database data importer loads into?
A: If you are using a Neo4j instance that supports multiple databases, Data Importer will use the home database to import data into. 
